...
describe("Mermaid", () => {
  ...
  it("renders with diagram change", () => {
    const config = {};
    jest.useFakeTimers();
    const view = expectMermaidMatch(
      render(<Mermaid chart={diagram} config={config} />)
    );
    view.rerender(
      <Mermaid
        chart={`graph TD;
  ...
  it("renders with mermaid config", () => {
    expectMermaidMatch(
      render(
        <Mermaid
          chart={diagram}
          config={{ mermaid: { theme: "dark" as mermaidAPI.Theme } }}
        />
      )
    );
    expect(spy.contentLoaded).toBeCalledTimes(1);
    expect(spy.initialize).toBeCalledWith({
      startOnLoad: true,
  ...
  it("renders with mermaid config change", () => {
    ...
    view.rerender(
      <Mermaid
        chart={diagram}
        config={{ mermaid: { theme: "forest" as mermaidAPI.Theme } }}
      />
    );
    // await waitFor(1000)
    expectMermaidMatch(view);
    expect(spy.contentLoaded).toBeCalledTimes(2);
    expect(spy.initialize).toHaveBeenNthCalledWith(2, {
      startOnLoad: true,
      theme: "forest",
    });
  });
  ...
  it("renders with string mermaid config", () => {
    expectMermaidMatch(
      render(
        <Mermaid
          chart={diagram}
          config={JSON.stringify({ mermaid: { theme: "dark" } })}
        />
      )
    );
    expect(spy.contentLoaded).toBeCalledTimes(1);
  ...
  it("re-renders mermaid theme on html data-theme attribute change", () => {
    ...
    act(() =>
      document
        .querySelector("html")!
        .setAttribute(HTML_THEME_ATTRIBUTE, DARK_THEME_KEY)
    );
    ...
    act(() =>
      document
        .querySelector("html")!
        .setAttribute(HTML_THEME_ATTRIBUTE, LIGHT_THEME_KEY)
    );
  ...
  it("does not react to non-theme attribute changes of html", () => {
    ...
    act(() =>
      document.querySelector("html")!.setAttribute("manifest", "some-value")
    );
